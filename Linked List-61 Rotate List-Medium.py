...
def rotateRight(head,k):
    ...
    # so if we put k%length 6%5 gives us 1 so we have rotate only 1 time
    curr=head # to take numbers other than k numbers initialize temp=> curr
    if k==0: #edge case
        return head
    for i in range(length-k-1):#y -1? without -1 it will jup three times and stop at 4th node so -1 will stop at 3rd node
        curr=curr.next
    NewHead=curr.next #making 4 as new head
    curr.next=None #3.next=None
    tail.next=head #connecting tail 5 we done on top to 1
    return NewHead
